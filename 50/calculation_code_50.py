...
def generate_one_subfield(subfield_id, sub_row, sub_col, global_start_id):
    ...
    # The y direction uses 18 rows, giving a 36x18 normal subfield.
    regions = [
        range(0, 12),  # region 1
...
def generate_wang_pattern_subfield(
    ...
    1. Left normal area: same 50% y-row sampling as normal subfields.
    2. 中间区域：王字形，保持原来的王字形写入逻辑
    3. Right normal area: same 50% y-row sampling as normal subfields.

    注意：
    - 王字形代码本身不改动
    - Left and right normal areas use the 50% normal-subfield y positions.
...
def plot_normal_subfield_order(all_results, target_subfield_id=1, save_name="normal_subfield_order_subfield1.png"):
    """
    Plot the writing order inside a normal 50% density subfield.
    The normal subfield uses 36 x-columns and 18 y-rows. The first 24
    subfields share this same internal pattern, so subfield 1 is enough for
    checking the normal order.
    """
    ...
    ax.set_title(f"Normal subfield {target_subfield_id}: 36x18 writing order")
    ax.set_xlabel("x local / um")
    ax.set_ylabel("y local / um")
...
# =====================
L_sub = 9.0  # 单个子场尺寸 (um)
N = 18  # Number of y rows kept in the 50% density normal subfield.
beam_L = 0.25  # 束斑尺寸 (um)
half_L = beam_L / 2
...
# 50% density keeps all 36 x-columns and 18 y-rows.
# =====================
