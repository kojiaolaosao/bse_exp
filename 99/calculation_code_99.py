...
def generate_one_subfield(subfield_id, sub_row, sub_col, global_start_id):
    ...
    # The y direction uses all 36 rows, giving a full 36x36 normal subfield.
    regions = [
        range(0, 12),  # region 1
...
def generate_wang_pattern_subfield(
    ...
    1. Left normal area: same full y-row coverage as normal subfields.
    2. 中间区域：王字形，保持原来的王字形写入逻辑
    3. Right normal area: same full y-row coverage as normal subfields.

    注意：
    - 王字形代码本身不改动
    - Left and right normal areas use the full normal-subfield y positions.
...
def plot_normal_subfield_order(all_results, target_subfield_id=1, save_name="normal_subfield_order_subfield1.png"):
    """
    Plot the writing order inside a normal 99% density subfield.
    The normal subfield uses the full 36x36 grid. The first 24 subfields share
    this same internal pattern, so subfield 1 is enough for checking the
    normal order.
    """
    ...
    ax.set_title(f"Normal subfield {target_subfield_id}: 36x36 writing order")
    ax.set_xlabel("x local / um")
    ax.set_ylabel("y local / um")
...
# 99% density uses the full 36x36 grid.
# =====================
